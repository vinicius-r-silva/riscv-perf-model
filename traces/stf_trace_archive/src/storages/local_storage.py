...
class LocalStorage(StorageHandler):
    ...
    def update_traces_table(self) -> pd.DataFrame:
        ...
        for workload_id in workload_ids:
            ...
            for trace_attempt in trace_attempts:
                ...
                metadata_files = [
                    filename for filename in trace_files if filename.endswith(self.metadata_suffix)]
                trace_ids = [
                    metadata_file[0:-len(self.metadata_suffix) - 1] for metadata_file in metadata_files]
                ...
                sample_trace_interval = sample_metadata.get(
                    'stf', {}).get('trace_interval', None)
                fully_trace = True if sample_trace_interval is None else False
                workload_sha = sample_metadata.get(
                    'workload', {}).get('SHA256', None)
    ...
    def insert_trace(self, trace_path: str, metadata: Metadata) -> None:
        ...
        storage_path = os.path.join(
            self.storage_path, workload_folder, attempt_folder)
        trace_storage_path = os.path.join(
            storage_path, f"{trace_id}.{self.trace_suffix}")
        metadata_storage_path = os.path.join(
            storage_path, f"{trace_id}.{self.metadata_suffix}")
    ...
    def get_metadata(self, trace_id: str) -> Metadata:
        ...
        metadata_path = os.path.join(
            self.storage_path, workload_folder, attempt_folder, f"{trace_id}.{self.metadata_suffix}")
    ...
    def save_trace(self, trace_id: str, path: str) -> None:
        ...
        trace_path = os.path.join(
            self.storage_path, workload_folder, attempt_folder, trace_filename)
    ...
    def save_workload(self, workload_id: int, path: str) -> None:
        workload_folder = os.path.join(
            self.storage_path, self._get_workload_folder(workload_id))
    ...
    def _get_workload_folder(self, workload_id: Union[str, int], workload_name: Optional[str] = None) -> str:
        ...
        if not workload_name:
            workload_name = self.workloads_table[self.workloads_table[WorkloadsTableSchema.WORKLOAD_ID]
                                                 == workload_id][WorkloadsTableSchema.WORKLOAD_NAME].item()
